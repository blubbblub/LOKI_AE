...
int tt_f2i(double dt, long int nxyz, long int nsta, double tp[nxyz][nsta], double ts[nxyz][nsta], int itp[nxyz][nsta], int its[nxyz][nsta], int nproc){
    ...
    for(i=0;i<nxyz;i++){
       /* tpmin may be different in different runs*/
       tpmin=tp[i][0];
       
       /* tpmin is the same after this step for each run*/
       for(j=0;j<nsta;j++){
           tpmin=min(tp[i][j],tpmin);
       }

       /* possible precision problem --> differnt itp/its afterwards
        ---> apparently no influence on location*/
       for(j=0;j<nsta;j++){
